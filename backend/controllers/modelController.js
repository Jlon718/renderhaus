...
const { uploadModel, deleteModel: deleteCloudinaryFile } = require('../config/cloudinary');
...
const uploadModelFile = async (req, res) => {
  try {
    ...
    console.log('Starting Cloudinary upload for model file...');
    
    // Upload model file to Cloudinary
    ...
    if (error.message.includes('413') || error.message.includes('File too large') || error.message.includes('Request Entity Too Large') || error.message.includes('File size too large')) {
      errorMessage = 'File is too large for upload. Cloudinary free tier supports files up to 100MB. Please try with a smaller file.';
      statusCode = 413;
  ...
  try {
    ...
    if (req.file) {
      ...
      if (model.uploadcareId) {
        await deleteCloudinaryFile(model.uploadcareId);
      }
