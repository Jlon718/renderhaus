...
const Dashboard = () => {
  ...
  const handleModelSelect = useCallback((model) => {
    ...
    // Also update selectedObject for ObjectProperties panel
    setSelectedObject(modelWithDefaults);
  ...
  const handlePositionChange = useCallback((objectId, axis, value) => {
    setPlacedModels(prevModels => 
      prevModels.map(model => {
    ...
    if (selectedObject?.id === objectId) {
      ...
      setSelectedObject(prev => ({ ...prev, position: newPosition }));
    }
    
    console.log(`Position changed for object ${objectId}, ${axis}-axis:`, value);
  }, [selectedObject]);
