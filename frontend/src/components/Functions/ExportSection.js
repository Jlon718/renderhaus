...
const ExportSection = ({ placedModels = [] }) => {
  ...
  const [currentSuggestedModels, setCurrentSuggestedModels] = useState([]);
  ...
  const fetchAIDataForExport = async () => {
    ...
    try {
      ...
      if (furnitureResponse.ok) {
        ...
        suggestedModels = furnitureData.suggestions?.suggested_models || [];
        setCurrentAiSuggestions(suggestions);
        setCurrentColorSuggestions(colors);
        setCurrentSuggestedModels(suggestedModels);
      }
    } catch (error) {
      console.error('Error fetching AI data for export:', error);
    }

    return { suggestions, colors, suggestedModels };
  };
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      const { suggestions: aiSuggestions, colors: colorSuggestions, suggestedModels } = await fetchAIDataForExport();
      ...
      // Section: Recommended Furniture Models (from database)
      if (suggestedModels && suggestedModels.length > 0) {
        ...
        pdf.setFontSize(16);
        pdf.setFont(undefined, 'bold');
        pdf.text('Recommended Furniture Models', margin, yPosition);
        yPosition += 10;

        // Prepare table data for suggested models
        const headers = ['#', 'Model Name', 'Category', 'Reason'];
        const columnWidths = [15, 50, 35, 90]; // Total: 190
        
        const rows = suggestedModels.map((model, index) => [
          (index + 1).toString(),
          model.name || 'Unnamed Model',
          model.category || 'N/A',
          model.reason || 'Recommended for your design'
        ]);
      ...
      <div style={{ marginBottom: '20px' }}>
        <div style={{ 
          ...
          <div style={{ fontSize: '14px', color: '#555' }}>
            ...
            <div style={{ marginBottom: '8px', display: 'flex', alignItems: 'center' }}>
              <CubeIcon style={{ width: '16px', height: '16px', marginRight: '8px' }} />
              <strong>Recommended Models:</strong> {currentSuggestedModels.length} model(s)
            </div>
            <div style={{ marginBottom: '8px', display: 'flex', alignItems: 'center' }}>
              <SparklesIcon style={{ width: '16px', height: '16px', marginRight: '8px' }} />
